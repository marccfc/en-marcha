...
document.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (form.id === "idea-form") return await saveIdea(new FormData(form));
    if (form.id === "video-form") return await saveVideo(new FormData(form));
    if (form.id === "details-form") return await saveDetails(new FormData(form));
  } catch (error) {
